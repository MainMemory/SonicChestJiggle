...
FunctionPointer(void, sub_71F5E0, (NJS_OBJECT*), 0x71F5E0);
DataPointer(SonicCharObj2*, dword_1A51A9C, 0x1A51A9C);
DataArray(float, flt_1A51A00, 0x1A51A00, 12);
void ShadowJiggleHook(NJS_OBJECT* obj)
{
	sub_71F5E0(obj);
	if (obj == CharacterModels[66].Model)
	{
		NJS_VECTOR a3{};
...
FunctionPointer(void, sub_49C5B0, (NJS_OBJECT*), 0x49C5B0);
...
{
	sub_49C5B0(obj);
	if (obj == CharacterModels[66].Model || obj == CharacterModels[356].Model)
	{
		NJS_VECTOR a3{};
...
void ShadowDisplayHook(ObjectMaster* obj)
{
	NJS_CNK_MODEL* tm1 = nullptr;
	NJS_CNK_MODEL* tm2 = nullptr;
	if (obj->Data2.Character->CharID2 == Characters_Shadow && jiggledata[obj->Data2.Character->PlayerNum])
	{
		tm1 = CharacterModels[67].Model->child->chunkmodel;
		CharacterModels[67].Model->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->TorsoJiggle->SourceModelCopy->chunkmodel;
		tm2 = CharacterModels[67].Model->child->child->chunkmodel;
		CharacterModels[67].Model->child->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->TorsoJiggle2->SourceModelCopy->chunkmodel;
	}
	Sonic_Display(obj);
	if (obj->Data2.Character->CharID2 == Characters_Shadow && jiggledata[obj->Data2.Character->PlayerNum])
	{
		CharacterModels[67].Model->child->chunkmodel = tm1;
		CharacterModels[67].Model->child->child->chunkmodel = tm2;
	}
}
...
void SuperDisplayHook(ObjectMaster* obj)
{
	...
	if (obj->Data2.Character->CharID2 == Characters_SuperShadow && jiggledata[obj->Data2.Character->PlayerNum])
	{
		tm1 = CharacterModels[67].Model->child->chunkmodel;
		CharacterModels[67].Model->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->TorsoJiggle->SourceModelCopy->chunkmodel;
		tm2 = CharacterModels[67].Model->child->child->chunkmodel;
		CharacterModels[67].Model->child->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->TorsoJiggle2->SourceModelCopy->chunkmodel;
		sm1 = CharacterModels[357].Model->child->chunkmodel;
		CharacterModels[357].Model->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->SuperTorsoJiggle->SourceModelCopy->chunkmodel;
		sm2 = CharacterModels[357].Model->child->child->chunkmodel;
		CharacterModels[357].Model->child->child->chunkmodel = jiggledata[obj->Data2.Character->PlayerNum]->SuperTorsoJiggle2->SourceModelCopy->chunkmodel;
	}
	Super_Display(obj);
	if (obj->Data2.Character->CharID2 == Characters_SuperShadow && jiggledata[obj->Data2.Character->PlayerNum])
	{
		CharacterModels[67].Model->child->chunkmodel = tm1;
		CharacterModels[67].Model->child->child->chunkmodel = tm2;
		CharacterModels[357].Model->child->chunkmodel = sm1;
		CharacterModels[357].Model->child->child->chunkmodel = sm2;
	}
}
...
void SuperLoadHook()
{
	...
	jiggledata[0]->TorsoJiggle2 = InitJiggle(CharacterModels[67].Model->child->child);
	...
	jiggledata[0]->TorsoJiggle2->Model = CharacterModels[67].Model->child->child;
	jiggledata[0]->TorsoJiggle = InitJiggle(CharacterModels[67].Model->child);
	...
	jiggledata[0]->TorsoJiggle->Model = CharacterModels[67].Model->child;
	jiggledata[0]->SuperTorsoJiggle2 = InitJiggle(CharacterModels[357].Model->child->child);
	...
	jiggledata[0]->SuperTorsoJiggle2->Model = CharacterModels[357].Model->child->child;
	jiggledata[0]->SuperTorsoJiggle = InitJiggle(CharacterModels[357].Model->child);
	...
	jiggledata[0]->SuperTorsoJiggle->Model = CharacterModels[357].Model->child;
...
extern "C"
{
		...
		for (int i = 0; i < 2; i++)
			if (MainCharObj2[i] && (MainCharObj2[i]->CharID2 == Characters_Shadow || MainCharObj2[i]->CharID2 == Characters_SuperShadow))
			{
				if (!jiggledata[i])
				{
					...
					jiggledata[i]->TorsoJiggle2 = InitJiggle(CharacterModels[67].Model->child->child);
					...
					jiggledata[i]->TorsoJiggle2->Model = CharacterModels[67].Model->child->child;
					jiggledata[i]->TorsoJiggle = InitJiggle(CharacterModels[67].Model->child);
					...
					jiggledata[i]->TorsoJiggle->Model = CharacterModels[67].Model->child;
					if (MainCharObj2[i]->CharID2 == Characters_SuperShadow)
					{
						jiggledata[i]->SuperTorsoJiggle2 = InitJiggle(CharacterModels[357].Model->child->child);
						...
						jiggledata[i]->SuperTorsoJiggle2->Model = CharacterModels[357].Model->child->child;
						jiggledata[i]->SuperTorsoJiggle = InitJiggle(CharacterModels[357].Model->child);
						...
						jiggledata[i]->SuperTorsoJiggle->Model = CharacterModels[357].Model->child;
					}
	...
	{
		WriteData((void (**)(NJS_OBJECT*))0x7201DB, ShadowJiggleHook);
		WriteData((void (**)(NJS_OBJECT*))0x49CB13, SuperJiggleHook);

		WriteData((void (**)(ObjectMaster*))0x7175CF, ShadowDisplayHook);
		WriteData((void (**)(ObjectMaster*))0x49ADB3, SuperDisplayHook);

		WriteCall((void*)0x498AB6, SuperLoadHook);
	}
